...
#ifndef _RESOLVE_KSYMS_H_
#define _RESOLVE_KSYMS_H_
...
unsigned long rk_kprobe_get_func_addr(const char *func_name) {
    ...
    if (register_kprobe(&kp) < 0) {
        printk(KERN_DEBUG "resolve_ksyms: register_kprobe for func %s failed\n", func_name);
        return -ENOENT;
    }

    unsigned long tmp = kp.addr;
    unregister_kprobe(&kp);
    printk(KERN_DEBUG "resolve_ksyms: register_kprobe found func %s @ %pK\n", func_name, tmp);
    return tmp;
}
...
unsigned long rk_kallsyms_lookup_name(const char *symbol_name) {
    ...
    printk(KERN_DEBUG "resolve_ksyms: kallsyms_lookup_name found func %s @ %pK\n", symbol_name, tmp);
    return tmp;
}
